...
type Service interface {
	FetchCompletionData(client *ssh.Client, userID uuid.UUID, serverID uuid.UUID, historyLimit int) (*CompletionData, error)
...
type service struct {
	...
	cache        map[cacheKey]*cacheEntry // 按 (userID, serverID) 缓存
...
func (s *service) FetchCompletionData(client *ssh.Client, userID uuid.UUID, serverID uuid.UUID, historyLimit int) (*CompletionData, error) {
	// 1. 尝试从缓存获取
	key := cacheKey{userID: userID, serverID: serverID}
	...
	// 3. 获取历史命令
	history, err := FetchHistory(client, shellType, historyLimit)
	if err != nil {
...
func (s *service) ClearCache(userID uuid.UUID, serverID uuid.UUID) {
	...
	key := cacheKey{userID: userID, serverID: serverID}
	delete(s.cache, key)
	log.Printf("Cleared cache for user: %s, server: %s", userID, serverID)
}
