...
// GetAllServersResources 获取所有服务器的资源概览
func (s *service) GetAllServersResources(ctx context.Context) (*AllServersResources, error) {
	...
	// 获取用户的所有服务器（使用较大的 limit）
	servers, _, err := s.serverService.List(ctx, userID, 1000, 0)
	if err != nil {
	...
	for i, srv := range servers {
		...
		go func(index int, srv *server.Server) {
			defer wg.Done()
			results[index] = s.collectServerResource(ctx, srv)
		}(i, srv)
	}
...
// StreamServersResources 流式获取服务器资源（每台服务器采集完成立即返回）
func (s *service) StreamServersResources(ctx context.Context, resultChan chan<- *ServerResourceSummary) error {
	...
	for _, srv := range servers {
		...
		go func(srv *server.Server) {
			defer wg.Done()
			result := s.collectServerResource(ctx, srv)
			select {
...
func (s *service) collectServerResource(ctx context.Context, srv *server.Server) *ServerResourceSummary {
	...
	// 从 Server 表读取地理位置信息（已在创建/更新时查询并存储）
	...
	// 解析输出
	s.parseBatchOutput(output, result)
...
func (s *service) parseBatchOutput(output string, result *ServerResourceSummary) {
	sections := s.parseSections(output)

	// 解析 CPU
	if cpuData, ok := sections["CPU"]; ok {
		result.CPU = s.parseCPUSummary(cpuData)
	}

	// 解析内存
	if memData, ok := sections["MEMORY"]; ok {
		result.Memory = s.parseMemorySummary(memData)
	}

	// 解析磁盘
	if diskData, ok := sections["DISK"]; ok {
		result.Disk = s.parseDiskSummary(diskData)
	}

	// 解析网络
	if netData, ok := sections["NETWORK"]; ok {
		result.Network = s.parseNetworkSummary(netData)
	}
...
func (s *service) parseSections(output string) map[string]string {
...
func (s *service) parseCPUSummary(data string) *CPUSummary {
...
func (s *service) parseMemorySummary(data string) *MemorySummary {
...
func (s *service) parseDiskSummary(data string) *DiskSummary {
	result := &DiskSummary{}
...
func (s *service) parseNetworkSummary(data string) *NetworkSummary {
